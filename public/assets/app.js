...
const FAQS = [
  ['Does the hospital have to approve this?', 'Not formally. The professional comes in as the patient\'s guest, during visiting hours, the same as a relative would — there is no vendor application and nothing for you to file. What we do is call the nurses\' station beforehand and agree a window, so nobody arrives at a bad moment. If the unit says today is not the day, we reschedule at no cost. Some units (ICU, transplant, isolation) restrict visitors, and their answer is final.'],
