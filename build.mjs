...
/* The map outlines are derived too — regenerate them if this checkout or
   deployment did not bring them along. */
if (!present(path.join(DATA, 'us-states.json'))) {
    const { execFileSync } = await import('child_process');
    execFileSync('node', ['tools/build-map.mjs'], { stdio: 'inherit', env: { ...process.env, MAP_OUT: path.join(DATA, 'us-states.json') } });
}
...
<main class="container legal">
  ...
  <h2>What we ask</h2>
  <ul>
    <li>A current cosmetology, barbering, esthetics or nail licence in the state where you would work. Give us the
        number — we check it against your state board's public register ourselves, so there is nothing to photograph.</li>
    <li>Your own liability insurance, $1M per occurrence. Your insurer can email the certificate straight to us.</li>
    <li>A background check, run before your first visit by a licensed screening provider who verifies your identity
        directly. <strong>We never ask you to send a photo of an ID, a passport or a Social Security card</strong> — if any
        platform asks you to email those, that is your cue to stop.</li>
